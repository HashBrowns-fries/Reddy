...
def get_feed_detail(
    ...
    for attempt in range(3):
        try:
            ...
            logger.debug("Navigation retry #%d: %s", attempt, e)
            time.sleep(2 + random.random() * 3 + attempt * 2)
    else:
        raise RuntimeError("Navigation failed after 3 attempts")
...
def _check_page_accessible(page: Page, url: str = "") -> None:
    ...
    扫码验证场景：progressive backoff with multiple retries.
    """
    ...
    if _is_scan_qrcode_verification(text) and url:
        ...
        for i, wait in enumerate(wait_times):
            ...
            logger.warning(
                "QR verification triggered, waiting %ds (retry %d/%d)...",
                wait, i + 1, len(wait_times),
            )
            ...
            raise PageNotAccessibleError(
                "Anti-crawl verification triggered. "
                "Please open the note manually in Chrome to complete verification, then retry."
            )
