...
from .models import Feed, FilterOption
...
def search_feeds(
    ...
    feeds = _extract_feeds(page)
    if not feeds:
        logger.info("First extract empty, retrying after 3s...")
        time.sleep(3)
        _wait_for_initial_state(page, timeout=10.0)
        feeds = _extract_feeds(page)
...
def _extract_feeds(page: Page) -> list[Feed]:
    """Extract feed list from page __INITIAL_STATE__."""
...
def _wait_for_initial_state(page: Page, timeout: float = 15.0) -> None:
...
(() => {
    try {
        ...
        if (!s || !s.search || !s.search.feeds) return 0;
        const d = s.search.feeds.value !== undefined ? s.search.feeds.value : s.search.feeds._value;
        return d ? d.length : 0;
    } catch(e) { return 0; }
})()
""")
        if count and int(count) > 0:
            return
        time.sleep(0.5)
    logger.warning("等待 search feeds 超时 (%ss)", timeout)
